CLASS zcl_22ee015_util DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    TYPES: BEGIN OF ty_sales_hdr,
             salesdocument TYPE zmk_header_t-salesdocument,
           END OF ty_sales_hdr,
           BEGIN OF ty_sales_item,
             salesdocument    TYPE zmk_item_t-salesdocument,
             salesitemnumber  TYPE zmk_item_t-salesitemnumber,
    ...
    CLASS-METHODS get_instance
      RETURNING VALUE(ro_instance) TYPE REF TO zcl_22ee015_util.

    METHODS:
      set_hdr_value
        IMPORTING im_sales_hdr TYPE zmk_header_t
        EXPORTING ex_created   TYPE abap_boolean,

      get_hdr_value
        EXPORTING ex_sales_hdr TYPE zmk_header_t,

      set_itm_value
        IMPORTING im_sales_itm TYPE zmk_item_t
        EXPORTING ex_created   TYPE abap_boolean,

      get_itm_value
        EXPORTING ex_sales_itm TYPE zmk_item_t,
  ...
  PRIVATE SECTION.
    CLASS-DATA: gs_sales_hdr_buff    TYPE zmk_header_t,
                gs_sales_itm_buff    TYPE zmk_item_t,
                gt_sales_hdr_t_buff  TYPE tt_sales_hdr,
                gt_sales_itm_t_buff  TYPE tt_sales_item,
                gv_so_delete         TYPE abap_boolean.

    CLASS-DATA mo_instance TYPE REF TO zcl_22ee015_util.
ENDCLASS.

CLASS zcl_22ee015_util IMPLEMENTATION.
